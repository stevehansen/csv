...
namespace Csv
{
    ...
    public static partial class CsvReader
    {
        ...
        private static IList<MemoryText> SplitLine(MemoryText line, CsvOptions options, int capacity = -1)
        {
        ...
        private sealed class ReadLine : ICsvLine
        {
            ...
            internal IList<MemoryText> RawSplitLine
            {
                ...
                {
#if NET8_0_OR_GREATER
                    rawSplitLine ??= SplitLine(Raw.AsMemory(), options, headers?.Length ?? -1);
#else
                    rawSplitLine ??= SplitLine(Raw, options, headers?.Length ?? -1);
#endif
                    return rawSplitLine;
                }
            }
        ...
        private sealed class ReadLineSpan : ICsvLineSpan
        {
            ...
            internal IList<MemoryText> RawSplitLine => rawSplitLine ??= SplitLine(Raw.AsMemory(), options, headers?.Length ?? -1);
        ...
        private sealed class ReadLineSpanOptimized : ICsvLineSpan
        {
            ...
            internal IList<ReadOnlyMemory<char>> RawSplitLine => rawSplitLine ??= SplitLineOptimized(rawMemory, options, memoryOptions, headers?.Length ?? -1);
        ...
        private static IList<ReadOnlyMemory<char>> SplitLineOptimized(ReadOnlyMemory<char> line, CsvOptions options, CsvMemoryOptions memoryOptions, int capacity = -1)
        {
